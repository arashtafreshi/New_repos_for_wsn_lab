	...
	implementation {
		...
		task void Serialmsgsend(){
			...
			TempPacket->DestID=buffer.DestID;
			TempPacket->Temperature=buffer.Temperature;
			printf("Sending on Serial (Serialmsgsend) \n");
			printfflush();
		...
		event message_t* SerialReceive.receive(message_t* msg, void* payload, uint8_t len) {  
			...
			if (len == sizeof(NodeToNodeMsg_t)) { 
				...
				printf("Packet Received on serial!!:(serialReceive) \n");
				//_Seq++;
		...
		event void LogRead.readDone(void* buf, storage_len_t len, error_t err) {
			...
			if ( (len == sizeof(logentry_t)) && (buf == &m_entry) ) {
				...
				if (TOS_NODE_ID != 0) {				
					...
					printf("Ready to send on Serial (logReaddone) \n");
					printfflush();
					post Serialmsgsend();
				}			
			}
